...
import com.project.dao.interfaceDao.EngineTypeDao;
...
public class EngineTypeImplDao implements EngineTypeDao {

     private static volatile EngineTypeImplDao INSTANCE;
    ...
    public static EngineTypeImplDao getInstance() {
        if (INSTANCE == null) {
            synchronized (EngineTypeImplDao.class) {
                if (INSTANCE == null) {
                    INSTANCE = new EngineTypeImplDao();
                }
            }
        }
        return INSTANCE;
    ...
    private EngineTypeImplDao() {
    }

    @Override
    public Optional<EngineTypeEntity> findById(Integer id) {
         try (Connection connection = ConnectionManager.getConnection();
              PreparedStatement preparedStatement = connection.prepareStatement(FIND_ENGINE_TYPE_BY_ID_SQL)) {
            preparedStatement.setInt(1, id);
            ResultSet resultSet = preparedStatement.executeQuery();
            if (resultSet.next()) {
                return Optional.of(new EngineTypeEntity(resultSet.getInt(ENGINE_TYPE_ID),
    ...
    public List<EngineTypeEntity> findAll() {
        List<EngineTypeEntity> engineTypes = new ArrayList<>();
        try (Connection connection = ConnectionManager.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(FIND_ALL_ENGINE_TYPES_SQL);
             ResultSet resultSet = preparedStatement.executeQuery()) {
            while (resultSet.next()) {
                engineTypes.add(new EngineTypeEntity(resultSet.getInt(ENGINE_TYPE_ID),
                        resultSet.getString(ENGINE_TYPE_NAME)));
            }
            return engineTypes;
    ...
    @Override
    public EngineTypeEntity save(EngineTypeEntity engineType) {
        try (Connection connection = ConnectionManager.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(CREATE_ENGINE_TYPE_SQL, RETURN_GENERATED_KEYS)) {
    ...
    @Override
    public void update(EngineTypeEntity engineType) {
        try (Connection connection = ConnectionManager.getConnection();
