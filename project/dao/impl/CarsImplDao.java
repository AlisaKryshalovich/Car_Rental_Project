...
import com.project.dao.CarsDao;
...
public class CarsImplDao implements CarsDao {

    private static volatile CarsImplDao INSTANCE;

    private final ModelsImplDao modelDao = ModelsImplDao.getInstance();
    private final BodyTypesImplDao bodyTypeDao = BodyTypesImplDao.getInstance();
    private final TransmissionsImplDao transmissionDao = TransmissionsImplDao.getInstance();
    private final EngineTypesImplDao engineTypeDao = EngineTypesImplDao.getInstance();
    private final CarStatusImplDao carStatusDao = CarStatusImplDao.getInstance();
    private final BrandsImplDao brandDao = BrandsImplDao.getInstance();
    ...
    public static CarsImplDao getInstance() {
        if (INSTANCE == null) {
            synchronized (CarsImplDao.class) {
                if (INSTANCE == null) {
                    INSTANCE = new CarsImplDao();
                }
            }
        }
        return INSTANCE;
    ...
    @Override
    public Optional<CarsEntity> findById(Integer id) {
    ...
    private CarsEntity buildCar(ResultSet resultSet) throws SQLException {
    ...
    public List<CarsEntity> findAll() {
        try (Connection connection = ConnectionManager.getConnection();
            ...
            throw new CarsDaoException(e);
        }
    }

    @Override
    public CarsEntity save(CarsEntity car) {
        try (Connection connection = ConnectionManager.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(CREATE_CAR_SQL, Statement.RETURN_GENERATED_KEYS)) {
